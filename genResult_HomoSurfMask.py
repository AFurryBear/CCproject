...
for voxel in cc_voxel:
    ...
    for fibre_index in cc_dic[voxel]:
        if template=="NONE":
            ...
            HCPMMP_condition = (template == 'HCPMMP' and fibre_tri.loc[fibre_index, 'label_L'] == fibre_tri.loc[
                fibre_index, 'label_R'] + 180)
            BN_Atlas_condition= (template == 'BN_Atlas' and fibre_tri.loc[fibre_index,'label_L']==fibre_tri.loc[fibre_index,'label_R']+1)
            if HCPMMP_condition or BN_Atlas_condition:
    ...
    os.system('mkdir -p '+os.path.join(maindir,subj,'result/part2_result','Homo_'+template))
    tex_L.darrays[0].data,tex_R.darrays[0].data=tex_l,tex_r

    nib.save(tex_L,os.path.join(maindir , subj , 'result/part2_result' ,'Homo_'+template, subj + '_' + voxel + '_L.tex.gii'))
    nib.save(tex_R,os.path.join(maindir , subj , 'result/part2_result' ,'Homo_'+template, subj + '_' + voxel + '_R.tex.gii'))